...
class QRCodeFollower(Node):
    ...
    def __init__(self):
        ...
        self.kp_x = 0.1  # Gain proportionnel pour l'axe X (gauche-droite)
        self.kp_y = 0.1  # Gain proportionnel pour l'axe Y (haut-bas)
        self.kp_z = 0.1  # Gain proportionnel pour l'axe Z (avant-arrière)
    ...
    def calculate_control_command(self, qr_center_x, qr_center_y, qr_size, img_width, img_height):
        """Calcule les commandes de contrôle pour recentrer le QR code (version simplifiée)"""
        # Centre de l'image
        img_center_x = img_width / 2
        img_center_y = img_height / 2
        
        # Diviser l'image en zones (gauche/centre/droite et haut/centre/bas)
        # Zone de tolérance autour du centre (1/3 de l'image)
        zone_x = img_width / 3
        zone_y = img_height / 3
        ...
        # Contrôle horizontal (axe X du drone - mouvement gauche/droite)
        if qr_center_x < img_center_x - zone_x / 2:
            # QR code à gauche -> aller à gauche
            twist.linear.x = -30.0
            self.get_logger().info('QR à gauche')
        elif qr_center_x > img_center_x + zone_x / 2:
            # QR code à droite -> aller à droite
            twist.linear.x = 30.0
            self.get_logger().info('QR à droite')
        else:
        ...
        if qr_center_y < img_center_y - zone_y / 2:
            # QR code en haut -> monter
            twist.linear.z = 30.0
            self.get_logger().info('QR en haut')
        elif qr_center_y > img_center_y + zone_y / 2:
            # QR code en bas -> descendre
            twist.linear.z = -30.0
            self.get_logger().info('QR en bas')
        else:
            # QR code centré verticalement
            twist.linear.z = 0.0
        
        # Pas de mouvement avant/arrière pour l'instant
        twist.linear.y = 0.0
