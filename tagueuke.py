...
from sensor_msgs.msg import Joy, Status
...
class ManualControl(Node):
    def __init__(self):
        ...
        self.subscription = self.create_subscription(Status, '/drone/status', self.status_callback, 10)
    ...
    def status_callback(self, msg: Status):
        # Exemple de traitement du statut du drone
        altitude = msg.distance_tof
        self.get_logger().debug(f"Statut du drone reçu: Altitude à {altitude} cm")
        if altitude == 10 :
            return True
        return False
        ...
        for btn_index in range(len(buttons)):
            ...
            if pressed and not prev_pressed:
                ...
                if time_since_last >= self.button_debounce:
                    ...
                    if btn_index == self.BTN_TAKEOFF and self.status_callback(Status()):
                    ...
                    elif btn_index == self.BTN_LAND and not self.status_callback(Status()):
